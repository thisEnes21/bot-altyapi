...
module.exports = (client, Tags, Embed, Discord) => {
    ...
    client.on('guildMemberAdd', async member => {
        ...
        if (data.enabled) {
            ...
            if (!channel) return;
            const text = data.message.replace("%kullanıcı%", member.user).replace("%toplam_üye%", member.guild.memberCount);
            ...
            ctx.fillText("Sunucuya Hoşgeldin!", 275, 110);
    ...
    client.on('guildMemberRemove', async member => {
        ...
        if(data.enabled){
            const channel = member.guild.channels.cache.get(data.channel_id);
            if(!channel) return;
            const text = data.message.replace("%kullanıcı%", member).replace("%toplam_üye%", member.guild.memberCount)
            ...
            ctx.font = "36px Sans-serif";
            ctx.fillStyle= "#f0f0f0";
            ctx.fillText("Sunucudan Ayrıldı.", 275, 130);

            // Text 2
            ctx.font = "32px Sans-serif";
            ctx.fillStyle = "#f0f0f0";
            ctx.fillText(member.displayName, canvas.width / 1.60 - (ctx.measureText(member.displayName).width / 2), 85);
